...
from pandocfilters import Link, RawInline, toJSONFilter
...
def process_link(text: str, key: str) -> tuple[str, str]:
    ...
    if key in inventories.pennylane.keys():
        return text if text else inventories.pennylane[
            key
        ].dispname, inventories.pennylane_base_url + inventories.pennylane[key].uri
    if key in inventories.catalyst.keys():
        return text if text else inventories.catalyst[
            key
        ].dispname, inventories.catalyst_base_url + inventories.catalyst[key].uri
    return text, key
    ...
    if key == "Code":
        ...
        if "interpreted-text" in classes:
            if "html" in role[0]:
                ...
                return Link(
                    ["", [], []], pandocify_string(name), [link.removesuffix("$"), ""]
                )
